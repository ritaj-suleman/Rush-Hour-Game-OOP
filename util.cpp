...
void DrawSquare(int sx, int sy, int size,const float color[]) {
	int mx=size,my=size;
...
* draw the triangle with the given color.
* */
void DrawCircle(float sx, float sy, float radius, const float*color) {
...
void DrawLine(int x1, int y1, int x2, int y2, int lwidth,const float *color) {
	glLineWidth(lwidth);
	if (color)
		glColor3fv(color);
	glBegin(GL_LINES);
...
void DrawString(float x, float y, const string& score, const float * color) {
...
void DrawRoundRect(float x, float y, float width, float height,const float* color,
	float radius) {
	y = y + height;
...
void DrawRectangle(int sx, int sy, int mx, int my, const float *color) {
